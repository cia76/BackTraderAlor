...
from AlorPy.Config import Config, ConfigIIA  # Конфигурация торгового счета / ИИС
from BackTraderMulti import MTBroker  # Мультиброкер
...
class MultiPortfolio(bt.Strategy):
    ...
    def start(self):
        ...
        print('- Стоимость позиций :', '%.2f' % value)
        print('- Свободные средства:', '%.2f' % cash)
        print('                     ', '%.2f' % (value + cash))

        stats = {'Фондовый рынок': {'alor_trade': Config.PortfolioStocks, 'alor_iia': ConfigIIA.PortfolioStocks},
                 'Срочный рынок': {'alor_trade': Config.PortfolioFutures, 'alor_iia': ConfigIIA.PortfolioFutures}}
        for market, providers_portfolios in stats.items():  # Пробегаемся по заданным рынкам, провайдерам/портфелям
            for provider, portfolio in providers_portfolios.items():  # Пробегаемся по всем провайдерам/портфелям рынка
                value = self.broker.getvalue(provider_name=provider, portfolio=portfolio)
                cash = self.broker.getcash(provider_name=provider, portfolio=portfolio)
                print(f'\n{market} ({portfolio})')
                print(f'- Стоимость позиций : {value:.2f}')
                print(f'- Свободные средства: {cash:.2f}')
                print(f'                      {(value + cash):.2f}')

        for d in self.datas:  # Пробегаемся по всем тикерам
            # Если по тикеру нет позиции, то будет Ошибка сервера: 404 Symbol <symbol> does not exist! <PreparedRequest [GET]>
            print('\nБаланс по тикеру', d._name, ':', '%.2f\n' % self.broker.getvalue(datas=(d,)))
    ...
    def next(self):
        ...
        for d in self.datas:  # По этим тикерам будет работать с заявками
            ...
            if not self.getposition(d):  # Если позиции нет
                ...
                size = 10 if d._name == self.datas[0]._name else 1  # Выставляем 10 акций / 1 фьючерс
    ...
    def notify_data(self, data, status, *args, **kwargs):
        ...
        print(f'{data._name} - {data_status}', datetime.now())  # Не можем вывести в лог, т.к. первый статус DELAYED получаем до первого бара (и его даты)
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    symbol_stocks = 'MOEX.SBER'  # Акция
    symbol_futures = 'MOEX.SI-6.23'  # Фьючерс <Код тикера заглавными буквами>-<Месяц экспирации: 3, 6, 9, 12>.<Последние 2 цифры года>
    ...
    store = ALStore(providers=[  # Хранилище Alor
        dict(provider_name='alor_trade', username=Config.UserName, refresh_token=Config.RefreshToken),  # Торговый счет Alor
        dict(provider_name='alor_iia', username=ConfigIIA.UserName, refresh_token=ConfigIIA.RefreshToken)])  # ИИС Alor
    broker_trade = store.getbroker(provider_name='alor_trade', use_positions=False, boards=Config.Boards, accounts=Config.Accounts)  # Брокер для торгового счета Alor
    broker_iia = store.getbroker(provider_name='alor_iia', use_positions=False, boards=ConfigIIA.Boards, accounts=ConfigIIA.Accounts)  # Брокер для торгового счета Alor
    cerebro.setbroker(MTBroker((broker_trade, broker_iia,)))  # Мультиброкер
    data_stocks = store.getdata(provider_name='alor_iia', dataname=symbol_stocks, timeframe=bt.TimeFrame.Minutes, compression=1, fromdate=datetime(2023, 3, 10),
                                sessionstart=time(10, 5), sessionend=time(18, 35), live_bars=True)  # Акцию будем торговать на ИИС
    cerebro.adddata(data_stocks)  # Добавляем акцию
    data_futures = store.getdata(provider_name='alor_trade', dataname=symbol_futures, timeframe=bt.TimeFrame.Minutes, compression=1, fromdate=datetime(2023, 3, 10),
                                 sessionstart=time(10, 5), sessionend=time(18, 35), live_bars=True)  # Фьючерс будем торговать на торговом счете
    cerebro.adddata(data_futures)  # Добавляем фьючер
