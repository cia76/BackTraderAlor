...
class LimitCancel(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        exchange = order.info['exchange']  # Биржа
        if order.status in (bt.Order.Created, bt.Order.Submitted, bt.Order.Accepted):  # Если заявка создана, отправлена брокеру, принята брокером (не исполнена)
            logger.info(f'Заявка {order.ref} на бирже {exchange} со статусом {order.getstatusname()}')
            ...
            logger.info(f'Заявка {order.ref} на бирже {exchange} отменена со статусом {order.getstatusname()}')
        elif order.status == bt.Order.Partial:  # Если заявка частично исполнена
            logger.info(f'Заявка {order.ref} на бирже {exchange} частично исполнена со статусом {order.getstatusname()}')
        elif order.status == bt.Order.Completed:  # Если заявка полностью исполнена
            logger.info(f'Заявка на {"покупку" if order.isbuy() else "продажу"} на бирже {exchange} исполнена по цене {order.executed.price}, Стоимость {order.executed.value}, Комиссия {order.executed.comm}')
